...
const deleteFeedFollowForUserIdAndFeedId = `-- name: DeleteFeedFollowForUserIdAndFeedId :exec
DELETE FROM feed_follows
WHERE feed_follows.user_id = $1
AND feed_follows.feed_id = $2
`

type DeleteFeedFollowForUserIdAndFeedIdParams struct {
	UserID uuid.UUID
	FeedID uuid.UUID
}

func (q *Queries) DeleteFeedFollowForUserIdAndFeedId(ctx context.Context, arg DeleteFeedFollowForUserIdAndFeedIdParams) error {
	_, err := q.db.ExecContext(ctx, deleteFeedFollowForUserIdAndFeedId, arg.UserID, arg.FeedID)
	return err
}
